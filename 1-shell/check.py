...
def run(writer=None):
    basic_tests = [
        ...
        TestGroup("Simple commands", 1.0,
                  ...
                  Test("Wait for 1 proc", test_wait_basic),
                  stop_if_fail=True),
    ...
    advanced_tests = [
        TestGroup("Pipes >2p with seqs or pipes", 1.0,
                  Test("Multiple pipes",
                       bash_cmp("ls -alh | grep .c | grep -v shell | tac")),
                  Test("Seq in pipe",
...
def check_compile():
    check_cmd(f"make moreclean ADDITIONAL_SOURCES='{additional_sources}'".encode("UTF-8"))

    out, err = check_cmd(f"make ADDITIONAL_SOURCES='{additional_sources}'".encode("UTF-8"))
    err = '\n'.join([l for l in err.split("\n") if not l.startswith("make:")])
    if "warning" in err:
        global compiler_warnings
        compiler_warnings = err

    check_cmd(f"{STUDENT_SHELL} -h".encode("UTF-8"))
...
def test_prompt(prompt):
    ...
    def test_prompt_inner():
        ...
        p.expect("\\$")
        if p.before + b"$" != expected_prompt().encode("UTF-8"):
            raise TestError("Prompt incorrect for \"%s\", expected \"%s\", got "
                            "\"%s\"" % (prompt, expected_prompt(), p.before.decode('UTF-8') + "$"))
        if "\\w" in prompt:
            p.sendline("cd /")
            p.expect("\\$")
            lines = p.before.decode("UTF-8").split("\r\n")
            if lines[0] != "cd /":
...
def sleep_status(p):
    ...
    for line in p.before.decode("UTF-8").split('\n'):
        if 'sleep' in line:
            return line.split()[2]
...
def test_wait_basic():
    global last_command
    last_command = "sleep 3"

    p = pexpect.spawn(STUDENT_SHELL)
    p.expect("\\$")
    stime = time.time()
    p.sendline(last_command)
    p.expect("\\$")
    if time.time() - stime < 0.3:
        raise TestError("'sleep 3' did not sleep for three seconds.")
...
def test_ctrl_c():
    ...
    try:
        ...
        p.sendline("ps")
        p.expect('\\$')
        if b"PID" not in p.before:
            raise TestError("Got no output from executing 'ps'.")
        if b"sleep" in p.before:
